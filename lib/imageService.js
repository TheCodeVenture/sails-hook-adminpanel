...
const mongoconnection = require('../../config/connections').connections[sails.config.models.connection]
...
const download = (res, pictureFd) => {
  if (isDEV) {
    return mongodb.MongoClient.connect(uri, function (err, db) {
      ...
      bucket.openDownloadStreamByName(pictureFd)
        .pipe(res)
